...
def active_cases_chart(root):
    ...
    line_info = plot_line(active_cases_date, active_cases, 'white', 'Currently Infected', ax, fig, 'Active Cases')
    plt.xticks(rotation=45)
    canvas = FigureCanvasTkAgg(fig, master=root)
    plot_widget = canvas.get_tk_widget()
...
def total_cured_chart(root):
    ...
    plot_widget = canvas.get_tk_widget()
    fig.canvas.mpl_connect("motion_notify_event", lambda event: hover(event, line_info, fig, ax, datax))
    #lol2 = fig.canvas.mpl_connect("motion_notify_event", lambda event: hover(event, line_info))
    return plot_widget
...
def infected_vs_recovered_chart(root):
    ...
    ax.yaxis.set_major_formatter(plt.FuncFormatter(format_func))
    fig.set_size_inches(15, 8)
    line_info1, line_info2 = plot_double_line(date, daily_cured, date, daily_cases, 'white', 'white', 'New Recoviries', 'New Infected', ax, fig, 'New Cases vs. New Recoveries')
    plt.xticks(rotation=45)
    canvas = FigureCanvasTkAgg(fig, master=root)
    plot_widget = canvas.get_tk_widget()
    fig.canvas.mpl_connect("motion_notify_event", lambda event: hover(event, line_info1))
    fig.canvas.mpl_connect("motion_notify_event", lambda event: hover(event, line_info2))
    return plot_widget
...
def total_serious_and_critical_cases(root):
    ...
    ax.yaxis.set_major_formatter(plt.FuncFormatter(format_func))
    fig.set_size_inches(15, 8)
    line_info = plot_line(total_serious_and_critical_cases_date, total_serious_and_critical_cases, 'white', 'Serious and Critical Cases', ax, fig, 'Total Serious and Critical Cases')
    plt.xticks(rotation=45)
    canvas = FigureCanvasTkAgg(fig, master=root)
    plot_widget = canvas.get_tk_widget()
    fig.canvas.mpl_connect("motion_notify_event", lambda event: hover(event, line_info))
...
def outcome_of_total_close_cases(root):
    ...
    plt.xticks(x_ticks)
    fig.set_size_inches(15, 8)
    line_info1, line_info2 = plot_double_line(date, death_rate, date, recovery_rate, 'white', 'white', 'Death Rate', 'Recovery Rate', ax, fig, 'Outcome of total closed cases (recovery rate vs death rate)')
    plt.xticks(rotation=45)
    canvas = FigureCanvasTkAgg(fig, master=root)
    plot_widget = canvas.get_tk_widget()
    fig.canvas.mpl_connect("motion_notify_event", lambda event: hover(event, line_info1))
...
def total_death_chart(root):
    ...
    ax.yaxis.set_major_formatter(plt.FuncFormatter(format_func))
    fig.set_size_inches(15, 8)
    line_info = plot_line(total_death_date, total_death, 'white', 'Deaths', ax, fig, 'Total Deaths')
    plt.xticks(rotation=45)
    canvas = FigureCanvasTkAgg(fig, master=root)
    plot_widget = canvas.get_tk_widget()
    fig.canvas.mpl_connect("motion_notify_event", lambda event: hover(event, line_info))
